...
from fpsica import psi_ca, Request, Response
...
class PSICAKNN(OverlayNode):
    ...
    def create_psi_request_to_send(
        self,
        channel="KNNConstr",
    ):     
        message = {"CHANNEL": channel, "KNNRound": self.knn_round, "PSI_REQUEST": self.request.SerializeToString(),
                   "SIZE": self.othersInfo[self.uid]}
        return message
    

    def create_psi_response_to_send(
        self,
        request,
        channel="KNNConstr"
    ):
        ...
        _request = Request()
        _request.ParseFromString(request)

        response = self.psica_node.ProcessRequest(_request,self.str_profile)
        message = {"CHANNEL": channel, "KNNRound": self.knn_round, "PSI_RESPONSE": response.SerializeToString(),
                   "SIZE": self.othersInfo[self.uid]}
        return message
    
    def create_psi_result_to_send(
        self,
        response,
        size_other,
        channel="KNNConstr"
        ...
        _response = Response()
        _response.ParseFromString(response)
        intersection = self.psica_node.ProcessResponse(_response)
        size = self.othersInfo[self.uid]
        jaccard = intersection / (size + size_other - intersection)
        message = {"CHANNEL": channel, "KNNRound": self.knn_round, "PSI_JACCARD": jaccard}
    ...
    def receiver_thread(self):
        ...
        while True:
            ...
            if "PSI_REQUEST" in x[1]:
                logging.debug(
                    "A PSI_REQUEST message received from {} from KNNRound {}".format(
                        x[0], x[1]["KNNRound"]
                    )
                )
                ...
                message =(x[0],self.create_psi_response_to_send(x[1]["PSI_REQUEST"]))
                self.sending_queue.put(message)

                

            elif "PSI_RESPONSE" in x[1]:
                logging.debug(
                    "A PSI_RESPONSE message received from {} from KNNRound {}".format(
                        x[0], x[1]["KNNRound"]
                    )
                )
                to_send , jaccard = self.create_psi_result_to_send(x[1]["PSI_RESPONSE"],x[1]["SIZE"])
                message = (x[0],to_send)
                self.sending_queue.put(message)
                self.mutex.acquire()
            ...
            elif "PSI_JACCARD" in x[1]:
                logging.debug(
                    "A PSI_JACCARD message received from {} from KNNRound {}".format(
                        x[0], x[1]["KNNRound"]
                    )
                )

                self.mutex.acquire()
                self.similarities[x[0]] = x[1]["PSI_JACCARD"]
                self.mutex.release()
    ...
    def build_topology(self, knn_rounds=4, random_nodes=4):
        ...
        for round in range(knn_rounds):
            ...
            for receiver in self.current_candidates:
                to_send = self.create_psi_request_to_send()
            ...
            while not self.knn_round_end():
                time.sleep(0.001) #POLLING FREQUENCY
    ...
    def run(self):
        ...
        #self.profile ,self.profile_ratings  = self.local_data_to_profile()
        ...
        self.str_profile = [str(x) for x in self.profile]
        self.psica_node = psi_ca.Node.CreateWithNewKey()
        request = Request()
        request.ParseFromString(self.psica_node.CreateRequest(self.str_profile).SerializeToString())
